...
void merge(long n, T left[n], T right[n], T result[n*2], long start, long length) {
	...
	tareador_start_task("mg0");
        merge(n, left, right, result, start, length/2);
	tareador_end_task("mg0");
	tareador_start_task("mg1");
        merge(n, left, right, result, start + length/2, length/2);
	tareador_end_task("mg1");
    }
}
...
void multisort(long n, T data[n], T tmp[n]) {
    if (n >= MIN_SORT_SIZE*4L) {
        // Recursive decomposition
	tareador_start_task("ms0");
        multisort(n/4L, &data[0], &tmp[0]);
        tareador_end_task("ms0");
	tareador_start_task("ms1");
	multisort(n/4L, &data[n/4L], &tmp[n/4L]);
	tareador_end_task("ms1");
	tareador_start_task("ms2");
	multisort(n/4L, &data[n/2L], &tmp[n/2L]);
        tareador_end_task("ms2");
	tareador_start_task("ms3");
	multisort(n/4L, &data[3L*n/4L], &tmp[3L*n/4L]);
	tareador_end_task("ms3");


	tareador_start_task("mgms0");
        merge(n/4L, &data[0], &data[n/4L], &tmp[0], 0, n/2L);
	tareador_end_task("mgms0");
	tareador_start_task("mgms1");
	merge(n/4L, &data[n/2L], &data[3L*n/4L], &tmp[n/2L], 0, n/2L);
	tareador_end_task("mgms1");
	tareador_start_task("mgms2");
        merge(n/2L, &tmp[0], &tmp[n/2L], &data[0], 0, n);
	tareador_end_task("mgms2");
